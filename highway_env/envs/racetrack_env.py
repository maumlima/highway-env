...
class RacetrackEnv(AbstractEnv):
    ...
    def _rewards(self, action: np.ndarray) -> Dict[Text, float]:
        longitude, lateral = self.vehicle.lane.local_coordinates(self.vehicle.position)
        
        return {
            "lane_centering_reward": 2/(1+self.config["lane_centering_cost"]*np.sqrt(lateral**2 )),
    ...
    def _make_road(self) -> None:
        ...
        # 3 - Vertical Straight
        net.add_lane("c", "d", StraightLane([120, -20], [120, -70],
                                            line_types=(LineType.CONTINUOUS, LineType.NONE), width=5,
                                            speed_limit=speedlimits[3]))
        net.add_lane("c", "d", StraightLane([125, -20], [125, -70],
        ...
        # 6 - Slant
        net.add_lane("f", "g", StraightLane([55.7, -55.7], [35.7, -75.7],
                                            line_types=(LineType.CONTINUOUS, LineType.NONE), width=5,
                                            speed_limit=speedlimits[6]))
        net.add_lane("f", "g", StraightLane([59.3934, -59.2], [39.3934, -79.2],
                                            line_types=(LineType.STRIPED, LineType.CONTINUOUS), width=5,
                                            speed_limit=speedlimits[6]))
        ...
        radii4 = 25
        net.add_lane("g", "h",
                     CircularLane(center4, radii4, np.deg2rad(315), np.deg2rad(190), width=5,
                                  clockwise=False, line_types=(LineType.CONTINUOUS, LineType.NONE),
                                  speed_limit=speedlimits[7]))
        net.add_lane("g", "h",
                     CircularLane(center4, radii4+5, np.deg2rad(315), np.deg2rad(185), width=5,
                                  clockwise=False, line_types=(LineType.STRIPED, LineType.CONTINUOUS),
                                  speed_limit=speedlimits[7]))
        net.add_lane("h", "i", StraightLane([-6.5, -62], [-6.5, -5],
                                            line_types=(LineType.CONTINUOUS, LineType.NONE), width=5,
                                            speed_limit=speedlimits[6]))
        net.add_lane("h", "i", StraightLane([-11.5, -62], [-11.5, -5],
                                            line_types=(LineType.STRIPED, LineType.CONTINUOUS), width=5,
                                            speed_limit=speedlimits[6]))
        ...
        net.add_lane("i", "j",
                     CircularLane(center5, radii5+5, np.deg2rad(90), np.deg2rad(190), width=5,
                                  clockwise=True, line_types=(LineType.CONTINUOUS, LineType.STRIPED),
                                  speed_limit=speedlimits[8]))
        net.add_lane("i", "j",
                     CircularLane(center5, radii5, np.deg2rad(90), np.deg2rad(190), width=5,
                                  clockwise=True, line_types=(LineType.NONE, LineType.CONTINUOUS),
                                  speed_limit=speedlimits[8]))
        
        net.add_lane("j", "a", StraightLane([-1.5, 0], [42, 0],
                                            line_types=(LineType.CONTINUOUS, LineType.NONE), width=5,
                                            speed_limit=10))
        net.add_lane("j", "a", StraightLane([-1.5, 5], [42, 5],
                                            line_types=(LineType.STRIPED, LineType.CONTINUOUS), width=5,
                                            speed_limit=10))
    ...
    def _make_vehicles(self) -> None:
        ...
        # Other vehicles
        while len(self.road.vehicles) < self.config["other_vehicles"] :
